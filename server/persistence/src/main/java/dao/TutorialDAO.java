...
public class TutorialDAO extends AbstractDAO<Tutorial> {
    ...
    public List<Tutorial> filterByKeyword(String keyword) {
        ...
        criteriaQuery.select(rootTutorial).where(
                cb.and(cb.like(cb.upper(rootTutorial.get("keywords")), "%" + keyword.toUpperCase() + "%")),
    ...
    public List<Tutorial> getAllDraftTutorialsForUser(User user) {
        ...
        Query query = em.createQuery(
                "SELECT t FROM Tutorial t where t.isDraft=true and :user member of t.contactPersons");
        query.setParameter("user", user);
        return query.getResultList();
